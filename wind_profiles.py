...
def generate_wind_profile(num_profiles=10, altitude_range=(0, 500), force_range=(-5, 5)):
    ...
    def generate_smooth_profile():
        ...
        b = [0.15, 1.0]  # Numerator coefficients 
        a = [1, -1.2, 0.6]    # Denominator coefficients 
        ...
        for j in range(num_profiles):
            ...
            for i, altitude in enumerate(altitudes):
                if i < change_point:
                    wind_forces[j, i] = (altitude - altitude_range[0]) * (middle_alt_force_choice - low_alt_force_choice)/(altitudes[change_point] - altitude_range[0]) + low_alt_force_choice + random.uniform(0, 0.25)
                else:
                    wind_forces[j, i] = (altitude - altitudes[change_point]) * (high_alt_force_choice - middle_alt_force_choice)/(altitude_range[1] - altitudes[change_point]) + middle_alt_force_choice + random.uniform(0, 0.25)
        ...
        #     for i, altitude in enumerate(altitudes):
        #         if i < change_point:
        #             wind_forces.append(initial_direction * random.uniform(0, 1))  # Increasing wind in one direction
        #         else:
        #             wind_forces.append(-initial_direction * random.uniform(0, 1))  # Wind changes direction
        # else:
        #     # Wind stays in one direction (constant)
        #     direction = random.choice([1, -1])  # Random constant direction
        #     for i, altitude in enumerate(altitudes):
        #         wind_forces.append(direction * random.uniform(0, 1))  # Constant wind force in one direction
...
generate_wind_profile(num_profiles=20)

# Example usage: Assuming CSV files are in the current directory
plot_wind_profiles_from_csv(num_profiles=20)